...
function extractNameFromTemplate(value) {
    let regExpForName = /^Hello, /;
    value = value.replace(regExpForName, "");
...
function encodeToRot13(str) {
    ...
    for (let i = 0; i < str.length; i++) {
        if (str[i].charCodeAt() >= 97 && str[i].charCodeAt() <= 122) {
            ...
            if (charCode > 122) {
                result += String.fromCharCode(`${charCode - 26}`);
            } else {
...
function getCardId(value) {
    ...
    let suit = "♣♦♥♠";
    let numberPosition = position.indexOf(value.slice(0, -1));
    if (numberPosition > 10) numberPosition--;
    let numberSuit = suit.indexOf(value.slice(-1)) * 13;

    result = numberPosition + numberSuit;
    return result;
}
